    ...
    <header>
      {navigatorShape === "closed" && (
        <div className={classes.closed}>
          <h3>{lang.list_of_posts}</h3>
          <IconButton
            aria-label={lang.expand_the_list}
            className={classes.expand}
            onClick={expandOnClick}
            title={lang.expand_the_list}
          >
            <ExpandLessIcon />
          </IconButton>
        </div>
      ...
      {navigatorShape === "open" &&
        categoryFilter !== lang.all_posts && (
          <div className={classes.filter}>
            <small>{lang.active_category_filter}</small> <strong>{categoryFilter}</strong>
            <IconButton
              aria-label={lang.remove_filtering}
              className={classes.clear}
              onClick={removeFilter}
              title={lang.remove_filtering}
            >
              <CloseIcon />
            </IconButton>
          </div>
